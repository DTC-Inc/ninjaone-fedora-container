...
6. **systemd-in-container needs tmpfs `/run`+`/tmp` and privileged — but NOT a `/sys/fs/cgroup` bind.** Podman `--systemd=always` sets all of it up; the compose/Custom-App path declares the tmpfs mounts explicitly. Do **not** add `- /sys/fs/cgroup:/sys/fs/cgroup:rw` — that's the cgroup **v1** recipe, and on a v2 host (TrueNAS SCALE 25.x and every modern distro) it *breaks the boot*: Docker already defaults to `cgroupns=private` on v2 and privileged already mounts the container's own cgroup writable, so binding the host's tree over it leaves systemd reading `0::/` from `/proc/self/cgroup` while looking at the host root cgroup — it can't create `init.scope`, aborts with *Failed to allocate manager object*, and the container exits within a second of starting. Privileged is required regardless (cgroup, SMART via `/dev`, namespace handoff).
